...
describe('Search game', () => {
	test('Add 2 games, search for game', async done => {
		...
		await page.goto('http://localhost:8080/newGame', { timeout: 30000, waitUntil: 'load' })
		await page.type('input[name=title]', 'The world 2')
		await page.type('textarea[name=summary]', 'This is a special summary')
		await page.type('textarea[name=desc]', 'Description')
		await page.click('button[type=submit]')

		await page.goto('http://localhost:8080/', { timeout: 30000, waitUntil: 'load' })
		await page.waitForSelector('div[id=games]')

		expect( await page.evaluate( () => document.querySelector('div[id=games]').children.length ) )
			.toBe(2)

		await page.goto('http://localhost:8080/', { timeout: 30000, waitUntil: 'load' })
		await page.type('input[name=gameSearch]', 'special')
		await page.click('button[id=submitSearch]')

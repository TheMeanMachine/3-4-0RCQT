...
describe('Removing publisher', () => {
	test('Remove publisher', async done => {
		//start generating a trace file.
		await page.tracing.start({path: 'trace/removePublisher.json',screenshots: true})
		...
		await page.select('select[name=publisher]', '1')
		await page.click('button[id=addPublisherSubmit]')

		await page.goto('http://localhost:8080/game?gameID=1', { timeout: 30000, waitUntil: 'load' })
		await page.click('button[id=removePublisherSubmit]')

		//ASSERT
		//check that the user is taken to the homepage after attempting to login as the new user:
		await page.waitForSelector('div[id=publishersContent]')

		expect( await page.evaluate( () => document.querySelector('div[id=publishersContent]').children.length ) )
			.toBe(0)
